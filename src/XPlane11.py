# -*- coding: utf-8 -*-
"""
Created on Wed Aug 28 11:30:27 2024

@author: Ammar Abdurrauf
version: 11.0
update:
    use image data from YOLOv8 to land
    use new mapping function from pixel to meter
    estimate altitude from object detection label
"""

from src.constants11 import CENTER_X, CENTER_Y, LANDING_CENTER_X, LANDING_CENTER_Z
from src.constants11 import INITIAL_POS_X, INITIAL_POS_Z, MAX_ALTITUDE, DIST_X, DIST_Z
from src.constants11 import Pp2, Ip2, Dp2, Ke2, Pr2, Ir2, Dr2, Ka2, Py2, Iy2, Dy2, Kr2, Pv2, Dv2, Kv2
from src.constants11 import Pp3, Ip3, Dp3, Ke3, Pr3, Ir3, Dr3, Ka3, Py3, Iy3, Dy3, Kr3, Pv3, Dv3, Kv3
from src.constants11 import Pp1, Ip1, Dp1, Pr1, Ir1, Dr1, FLIGTH_DATA_DIR, FLIGTH_DATA_FILENAME, CONTROL_DATA_FILENAME, YOLO_DATA_FILENAME
...
class XPlane:
        ...
        with DXCamera(self.xplane_screen[0],
            ...
            while True:
                ...
                if len(boxes) > 0:
                    ...
                    self.height_px = boxes[index].xywh[0].tolist()[-1]
    ...
    def landing_with_yolo(self, target_altitude, dx_gps, dz_gps, altitude_gps):
        ...
        # altitude = altitude_gps
        if not self.target_center == None:
        ...
        self.yolo_data['height px'].append(self.height_px)
        self.yolo_data['real alt'].append(altitude_gps)
        self.yolo_data['predicted alt'].append(altitude)        
        self.yolo_data['predicted pos x'].append(dz)
        self.yolo_data['predicted pos y'].append(dx)
        
        print(f'self.height_px: {self.height_px}\naltitude_gps: {altitude_gps}\naltitude: {altitude}\ndx_gps: {dx_gps}\ndx: {dx}\ndz_gps: {dz_gps}\ndz: {dz}\n')
        
        # -----------------------------------------------------
        distance_to_landing_pad = sqrt((dx**2) + (dz**2))
        altitude_error = target_altitude - altitude + 0.5 * distance_to_landing_pad
        ...
        # if (abs(dx) > 3 and altitude < target_altitude):
        if (abs(dx) > 1.5):
            pitch_error = self._tanh(pitch_error - 0.6 * dx, a=30, b=0.05)
            pitch_dterror = pitch_dterror + 0.6 * self.vel_x
        ...
        roll_dterror = - 1.2 * self.roll_rate
        if (abs(dz) > 1.5):
            roll_error = self._tanh(roll_error + 0.6 * dz, a=30, b=0.05)
            roll_dterror = roll_dterror - 0.6 * self.vel_z
    ...
    def xplane_communication(self):
        ...
        while self.run:
            ...
            if stage == 1:
                ...
                throttle_values = self.landing_with_yolo(target_altitude, self.dx, self.dz, self.altitude)
        ...
        save_data_to_csv(FLIGTH_DATA_DIR, YOLO_DATA_FILENAME, self.yolo_data)
