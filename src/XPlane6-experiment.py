# -*- coding: utf-8 -*-
"""
Created on Fri Aug 23 21:33:06 2024
...
class XPlane:
    ...
    def set_env(self):
        ...
        self.starship.sendDREF('sim/flightmodel/position/q', [0.8, 0, 0, 0.82])
        # reposition the rocket
        self.starship.sendDREF('sim/flightmodel/position/local_x', LANDING_CENTER_X+100)
        self.starship.sendDREF('sim/flightmodel/position/local_z', LANDING_CENTER_Z+25)
    ...
    def _compute_pid_output(self, error, dt_error, 
                            accumulated_error,
                            Kp, Ki, Kd):
        accumulated_error = self._accumulate_intregral_error(error, accumulated_error)
        return (Kp * error + Ki * accumulated_error + Kd * dt_error), accumulated_error
    ...
    def xplane_communication(self):
        ...
        while self.run:
            ...
            if not hover:
                ...
                Dv = 0.112
                Kv = 0.4
                throttle_values, elevator, aileron = self.hover_to_landing_pad(self.altitude, ver_vel, Pv, Dv,
                                                                               Kv, Ke, Ka, dx, 
                                                                               vel_x, dz, vel_z,
                ...
                safe_for_engine_turn_off = abs(self.altitude) + abs(vel_x) + abs(vel_z) + abs(pitch)*0.2 + \
                                            abs(roll)*0.2 + abs(pitch_rate) + abs(roll_rate) + abs(ver_vel)
