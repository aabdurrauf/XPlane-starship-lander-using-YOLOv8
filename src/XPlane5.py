# -*- coding: utf-8 -*-
"""
Created on Sun Aug 18 19:54:32 2024

@author: Ammar Abdurrauf
version: 5.0
update:
    adding side thuster (use x-plane v2)
    hover
"""
...
class XPlane:
    ...
    def start_sim(self):
        ...
        # throttle on        
        throttle_values = [2, 2, 2, 0, 0, 0, 0, 0]
    ...
    def hover(self, ver_vel, Kv, pitch, pitch_rate, pitch_integral, roll, roll_rate, roll_integral, Pp, Ip, Dp, Pr, Ir, Dr):
        pitch_ref = 0
        roll_ref = 0
        ...
        thruster_3 = abs(min(max(pitch_error * Pp + pitch_rate * Dp, -1), 0) if pitch_rate < 0 else pitch_rate * Dp)
        thruster_4 = abs(max(min(pitch_error * Pp + pitch_rate * Dp, 1), 0) if pitch_rate > 0 else pitch_rate * Dp)
        thruster_5 = abs(min(max(roll_error * Pp + roll_rate * Dp, 1), 0) if roll_rate > 0 else roll_rate * Dr)
        thruster_6 = abs(min(max(roll_error * Pp + roll_rate * Dp, -1), 0) if roll_rate < 0 else roll_rate * Dr)
        
        # this also works but above code is more robust
        # div = 4
        # thruster_3 = abs(min(max(pitch_error * Pp + pitch_rate * Dp, -1), 0))
        # if thruster_3 == 0 and pitch_rate > 0:
        #     thruster_3 = abs(pitch_rate * Dp/div)
            
        # thruster_4 = abs(max(min(pitch_error * Pp + pitch_rate * Dp, 1), 0))
        # if thruster_4 == 0 and pitch_rate < 0:
        #     thruster_4 = abs(pitch_rate * Dp/div)
            
        # thruster_5 = abs(min(max(roll_error * Pp + roll_rate * Dp, 1), 0))
        # if thruster_5 == 0 and roll_rate < 0:
        #     thruster_5 = abs(roll_rate * Dp/div)
            
        # thruster_6 = abs(min(max(roll_error * Pp + roll_rate * Dp, -1), 0))
        # if thruster_6 == 0 and roll_rate > 0:
        #     thruster_6 = abs(roll_rate * Dp/div)
        
        main_engines = max(min(-ver_vel * Kv, 2), 0)
        print(f't_3: {thruster_3}\nt_4: {thruster_4}\nver_vel: {ver_vel}\nmain_engines: {main_engines}')
    ...
    def xplane_communication(self):
        ...
        while self.run:
            ...
            if not landing:
                ...
                Kv = 1
                throttle_values = self.hover(ver_vel, Kv,
