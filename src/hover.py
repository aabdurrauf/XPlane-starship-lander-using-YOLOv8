# -*- coding: utf-8 -*-
"""
Created on Mon Aug 19 20:28:14 2024
...
starship.sendDREF('sim/flightmodel/position/local_z', 
                  starship.client.getDREF('sim/flightmodel/position/local_z')[0] + 0)
starship.sendDREF('sim/flightmodel/position/local_x', 
                  starship.client.getDREF('sim/flightmodel/position/local_x')[0] + 0)
...
def hover(altitude, ver_vel, Pv, Dv, 
                         Kv, Ke, Ka,
                         dx, vel_x, dz, vel_z,
    ...
    elevator_side_thruster = pitch_error * Pp + pitch_integral * Ip + pitch_dterror * Dp
    # -----------------------------------------------------
    roll_ref = 0
    roll_error = roll_ref - 1 * roll + 0.2 * dz
    roll_dterror = -roll_rate - 0.8 * vel_z
    aileron_side_thruster = roll_error * Pr + roll_integral * Ir + roll_dterror * Dr
    # -----------------------------------------------------  
    pitch_ref = 0
...
while True:
    ...
    throttle_values, elevator, aileron = hover(altitude, ver_vel, Pv, Dv,
                                           Kv, Ke, Ka, dx, 
                                           vel_x, dz, vel_z,
                                           pitch, pitch_rate, pitch_integral,
                                           roll, roll_rate, roll_integral,
